    ...
    try:
        ...
        # give the expected result
            ...
            if baseCMD == "ls":
                ...
                message = constructMessage(str(result), "d", 200)
                print(message)
                sock.send(message.encode())
            if baseCMD == "get":
...
def handlePut(socket, userRequest):
    ...
    if response[2] == "directory":
        ...
        if not os.path.exists(userRequest['fileRequest']):
            ...
            output = ("Error: File already exists and cannot be overwritten.")
    elif response[1] == "data":
        print("Received Data:", response[0:])
